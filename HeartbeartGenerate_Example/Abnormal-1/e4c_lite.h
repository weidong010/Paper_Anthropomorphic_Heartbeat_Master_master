...
#ifndef E4C_MAX_CONTEXTS
# define E4C_MAX_CONTEXTS 1
#endif

/* Retrieve current exception context number */
#ifndef E4C_CONTEXT_NUMBER
# define E4C_CONTEXT_NUMBER 0
#endif
...
#define E4C_EXCEPTION e4c[E4C_CONTEXT_NUMBER].err
...
#define E4C_TRY if(e4c_try(E4C_CONTEXT_NUMBER, E4C_INFO) && setjmp(e4c[E4C_CONTEXT_NUMBER].jump[e4c[E4C_CONTEXT_NUMBER].frames - 1]) >= 0) while(e4c_hook(E4C_CONTEXT_NUMBER, 0)) if(e4c[E4C_CONTEXT_NUMBER].frame[e4c[E4C_CONTEXT_NUMBER].frames].stage == e4c_trying)
#define E4C_CATCH(type) else if(e4c[E4C_CONTEXT_NUMBER].frame[e4c[E4C_CONTEXT_NUMBER].frames].stage == e4c_catching && E4C_IS_INSTANCE_OF(type) && e4c_hook(E4C_CONTEXT_NUMBER, 1))
#define E4C_FINALLY else if(e4c[E4C_CONTEXT_NUMBER].frame[e4c[E4C_CONTEXT_NUMBER].frames].stage == e4c_finalizing)
#define E4C_THROW(type, message) e4c_throw(E4C_CONTEXT_NUMBER, &type, E4C_INFO, message)
#ifndef NDEBUG
...
extern struct e4c_context{jmp_buf jump[E4C_MAX_FRAMES]; struct e4c_exception err; struct{unsigned char stage; unsigned char uncaught;} frame[E4C_MAX_FRAMES + 1]; int frames;} e4c[E4C_MAX_CONTEXTS];
extern int e4c_try(int context, const char * file, int line);
extern int e4c_hook(int context, int is_catch);
extern int e4c_extends(const struct e4c_exception_type * child, const struct e4c_exception_type * parent);
extern void e4c_throw(int context, const struct e4c_exception_type * exception_type, const char * file, int line, const char * message);
